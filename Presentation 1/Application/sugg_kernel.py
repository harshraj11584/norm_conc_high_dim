...
def get_new_kernel_matrix(x1,x2,grid,p=1.0):
	...
	if kernel_mat.shape[0]==kernel_mat.shape[1]:
		lowest_eig_val = eigh(kernel_mat,eigvals_only=True,eigvals=(0,0))[0]
		print("lowest eigen value = ",lowest_eig_val)
	return kernel_mat
...
def experiment(dim):
	x_train,x_test,y_train,y_test = prep_data(n_samples=10000,n_features=dim)
